<template>
  <section class="py-10 glassmorphism">
    <SharedHeader title="about" />
    <p class="px-5 text-sm font-normal">
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum ipsam vitae
      nemo, commodi voluptates cumque earum consequuntur enim quia quibusdam
      omnis odio possimus doloribus sapiente ut. Doloremque repellat corrupti ea
      totam officia assumenda. Voluptatem ducimus corporis laborum incidunt,
      commodi fuga? Nemo quis iure temporibus. Repellendus aliquam ullam minima.
      Animi quia at illo nobis nihil cupiditate similique numquam eaque dolor
      maiores possimus autem fugit officia laboriosam quam, assumenda porro
      natus cum repellat soluta! Similique, debitis assumenda. Ipsam fugit,
      quaerat quod nemo maiores voluptate earum praesentium quo vero at
      molestias libero eligendi cupiditate ipsa possimus quos corporis pariatur
      cum accusamus. Suscipit, eligendi.
    </p>
  </section>
</template>

<script lang="ts" setup></script>

<style></style>
